...
RSpec.feature 'User #Show', type: :feature do
  ...
  scenario "show user's name" do
    find("a[href='#{user_path(@user1.id)}']").click
    expect(page).to have_content 'Munsa mibenge'
  end

  scenario 'show number of posts per user' do
    Post.create(title: 'How To unit test with Rspec', text: 'Try to check on Google.', user_id: @user1.id)
    Post.create(title: 'Avoiding procrastination', text: 'lorem ipsummffjglgirrin snsnsjdidor', user_id: @user1.id)
  ...
  scenario "show user's bio." do
    Post.create(title: 'Oversabi dey kill', text: 'lorem ipsummffjglgirrin snsnsjdidor', user_id: @user1.id)
  ...
  scenario "show user's first 3 posts." do
    Post.create(title: 'Let Us Win', text: 'Etiam et mauris et', user_id: @user1.id)
    Post.create(title: 'Rails Routes', text: 'et mauris et ligula', user_id: @user1.id)
    Post.create(title: 'Why I love Programming', text: 'Etiam et et ligula', user_id: @user1.id)
    Post.create(title: 'Love World', text: 'iam et et ligula', user_id: @user1.id)
    find("a[href='#{user_path(@user1.id)}']").click
  ...
  scenario "click post and redirect to that post's show page." do
    @post = Post.create(title: 'Lorem ipsum dolor', text: 'Etiam et mauris et', user_id: @user1.id)
    find("a[href='#{user_path(@user1.id)}']").click
